...
pipeline {
    ...
    // ---------- АВТОЗАПУСК ----------
    triggers {
        // Локальный Jenkins недоступен из интернета, поэтому webhook (githubPush)
        // не сработает. Вместо этого Jenkins сам раз в минуту проверяет GitHub.
        pollSCM('* * * * *')
        // cron('H 2 * * *')        // ежедневная ночная сборка
        // githubPush()             // включите обратно, если Jenkins станет доступен из интернета
    }
